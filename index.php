...
// -------------------------------------------------------------------------------
  ...
  if ($debug) 
  {
    ...
    echo "</head>\n";
    echo "<body>\n";
    echo "<h2>The code is not working right now. Sorry!</h2>\n";
  }
// -------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------
  ...
  if ($debug) echo "<p>Input: $get_date</p>\n";
  if ($get_date == "")              $get_date  = date("Y-m-d");      // Today is the default
  if ($debug) echo "<p>$get_date</p>\n";
...
// -------------------------------------------------------------------------------
  ...
  if(get_http_response_code("http://geo.crox.net/djia/$dow_date_w") != "200"){
    echo "<h1>No valid DJIA found for $dow_date_w - Sorry!</h1>";
    exit;
  }
  else
  {
...
// -------------------------------------------------------------------------------
  ...
  list($lat_w, $lon_w) = $calc->doCalc($get_date, $djia_w);
...
// -------------------------------------------------------------------------------
  ...
  if ($debug)
  {
    ...
    echo "<tr>\n";                                                                                   // Debug only
    echo "<td style=\"border-style:solid; border:solid 1px #bbb; padding:2px;\">\$get_skins</td>\n"; // Debug only
    echo "<td style=\"border-style:solid; border:solid 1px #bbb; padding:2px;\">\$get_date</td>\n";  // Debug only
    echo "<td style=\"border-style:solid; border:solid 1px #bbb; padding:2px;\">\$get_lat</td>\n";   // Debug only
    echo "<td style=\"border-style:solid; border:solid 1px #bbb; padding:2px;\">\$get_lon</td>\n";   // Debug only
  ...
  for ($yy_lat = -$get_skins; $yy_lat < ($get_skins + 1);  $yy_lat++) {      // Iterate through latitudes  (vertical)
    for ($xx_lon = -$get_skins; $xx_lon < ($get_skins + 1);  $xx_lon++) {    // Iterate through longitudes (horizontal)
      ...
      $kml .= "           <description><![CDATA[" . 
                            number_format($lat, 8) . " " . number_format($lon, 8) . "<br>" .
                            ...
                            "<a href=\"http://maps.google.com/?ie=UTF8&ll=" . $lat . "," . $lon . "&z=8&q=loc:" . $lat . "," . $lon . "\">Google Map</a>]]></description>\n";
      $kml .= "        <LookAt>\n";
